...
window.addEventListener('message', event => {
  ...
  switch (message.command) {
    ...
    case 'aiLoading':
      ...
      break;
    case 'aiResponse':
      renderAiResponse(message.text, message.actions, message.provenance);
      break;
...
function renderAiResponse(text, actions, provenance) {
  const cleanText = (window.ChatActions && ChatActions.stripActionsTailForDisplay)
    ? ChatActions.stripActionsTailForDisplay(text)
    : text;
  ...
  if (lastMsg && Array.isArray(actions) && actions.length > 0 && window.ChatActions) {
    const bubble = lastMsg.querySelector('.ai-message-bubble');
    if (bubble) {
      ChatActions.showSuggestionBubbles(actions, {
        container: aiResponseArea,
        stripId: 'dashboardBubbleStrip',
        assistantSelector: '.ai-message.assistant',
        provenance: provenance || { provenance: 'explicit', database: currentDbName },
        maxActions: 3,
        onAsk: (action) => {
          sendAiQuestion(action.prompt || action.label);
        },
        onAction: (action) => {
          if (action.kind === 'ask') {
            sendAiQuestion(action.prompt || action.label);
